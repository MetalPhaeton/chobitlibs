...
//    GRULayer::<OUT, IN>::new(
//        [0u8; OUT].iter().map(
//            |_| gen_gru_neuron::<IN>(rng)
//        ).collect::<Vec<GRUNeuron<IN>>>().try_into().unwrap(),
//        [0u8; OUT].iter().map(
//            |_| gen_weights_1::<OUT>(rng)
//        ).collect::<Vec<Weights<OUT>>>().try_into().unwrap()
//    )
//}
//
...
//                    vec.push([0u8; IN].iter().map(
//                        |_| (rng.next_u64() % 20) as f32
//                    ).collect::<Vec<f32>>().try_into().unwrap());
//                }
//                vec
//            }
...
//    rng: &mut ChobitRand
//) -> Layer<OUT, IN> {
//    Layer::<OUT, IN>::new([0u8; OUT].iter().map(
