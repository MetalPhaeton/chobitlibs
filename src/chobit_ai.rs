...
> MLEncoderCache<OUT, MIDDLE, IN> {
    ...
    pub fn new(capacity: usize) -> Self {
        Self {
            lstm_caches: vec![
                MLLSTMCache::<MIDDLE, IN>::new(); capacity
            ],
...
#[derive(Debug, Clone, PartialEq)]
pub struct Decoder<const OUT: usize, const MIDDLE: usize, const IN: usize> {
    lstm: LSTM<MIDDLE, IN>,
    output_layer: Layer<OUT, MIDDLE>
}
    ...
    Decoder<OUT, MIDDLE, IN>
{
    ...
    pub fn calc(
        ...
        output.iter_mut().for_each(|output_one| {
            ...
            self.output_layer.calc(middle_output, None, output_one);
...
pub struct MLDecoderCache<
    ...
    const IN: usize
> {
    caches: Vec<(MLLSTMCache<MIDDLE, IN>, MLCache<OUT, MIDDLE>)>,
    output_len: usize
}
impl<
    const OUT: usize,
...
> MLDecoderCache<OUT, MIDDLE, IN> {
    ...
    pub fn caches(
        &self
    ) -> &[(MLLSTMCache<MIDDLE, IN>, MLCache<OUT, MIDDLE>)] {
        &self.caches[..self.output_len]
    }
    ...
    pub fn calc_error(
        ...
        train_out.iter().zip(
            ...
            error
        ).for_each(|((train_out, (_, cache)), error)| {
            error.copy_from(cache.output());
            *error -= train_out;
        });
    }
}
...
#[derive(Debug, Clone, PartialEq)]
pub struct MLDecoder<const OUT: usize, const MIDDLE: usize, const IN: usize> {
    lstm: MLLSTM<MIDDLE, IN>,
    output_layer: MLLayer<OUT, MIDDLE>,
...
impl<
    ...
    const MIDDLE: usize,
    const IN: usize
> MLDecoder<OUT, MIDDLE, IN> {
    #[inline]
    pub fn new(decoder: Decoder<OUT, MIDDLE, IN>) -> Self {
        let Decoder::<OUT, MIDDLE, IN> {lstm, output_layer} = decoder;
    ...
    #[inline]
    pub fn drop(self) -> Decoder<OUT, MIDDLE, IN> {
        let Self {lstm, output_layer, ..} = self;
    ...
    pub fn ready<'a>(
        ...
        for (lstm_cache, output_layer_cache) in cache.caches.iter_mut() {
            if count >= output_len {
                cache.output_len = count;
                return;
            }

            self.lstm.ready(input, prev_cell, lstm_cache);
            ...
            prev_cell.copy_from(&lstm_cache.cell);
        }

        while count < output_len {
            let mut lstm_cache = MLLSTMCache::<MIDDLE, IN>::new();
            let mut output_layer_cache = MLCache::<OUT, MIDDLE>::new();

            self.lstm.ready(input, prev_cell, &mut lstm_cache);
            ...
            prev_cell.copy_from(&lstm_cache.cell);
            cache.caches.push((lstm_cache, output_layer_cache));
        }
    ...
    pub fn study<'a>(
        ...
        let mut output_error_and_cache_iter =
            output_error.iter().zip(&cache.caches).rev();

        match output_error_and_cache_iter.next() {
            Some((output_error, (lstm_cache, output_layer_cache))) => {
                self.output_layer.study(
                    output_error,
                    output_layer_cache,
                    &mut self.tmp_middle_error,
        ...
        output_error_and_cache_iter.for_each(
            |(output_error, (lstm_cache, output_layer_cache))| {
                self.output_layer.study(
                    output_error,
                    output_layer_cache,
                    &mut self.tmp_middle_error,
